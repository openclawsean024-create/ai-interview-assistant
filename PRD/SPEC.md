...
> 版本：v3.0（從 stub root SPEC.md v1.0 升級｜2026-07-19）｜維護者：Sean PRD Rewrite Specialist｜對接技術：Hermes Agent + engineering
> 文件狀態：v3.0 stub upgrade；在 v2.2.1 sweet-spot-driven rewrite 基礎上補強 North Star Metric / Stage Gate / 2 次 sweet spot re-check / 2026-07-19 stub upgrade 註記。
> 原始 root SPEC.md：233 bytes v1.0 stub（遠端實測為 473 bytes，schema 已含「產品概覽／目標用戶／核心功能／技術架構」四段，無 KPI、Non-Goals、Stage Gate、AC 等正式區塊）。
> 原始碼：https://github.com/openclawsean024-create/ai-interview-assistant
> sweet spot：5/10 → 二次 re-check 5/10（不變）→ **本版 stub upgrade 後 6/10**（依 2026-07-19 競品 quick check + PTT/Dcard/Threads 搜尋證據小幅上調）
> 建議動作：investigate → **INVESTIGATE（升級，千字補強 + Stage 1.5 gate，**不替前版 v2.2.1 結論翻案**）**

本文件的數字、競品與市場結論均為待驗證假設；不可把 mock、HTTP 可達性或訪談口頭意願當成營收事實。
v3.0 與 v2.2.1 的差異主要為「文件骨架升級」與「2026-07-19 重新評估」，不替換任何已建立的 FR/AC/ADR。
---
...
---
## 16. v3.0 Stub Upgrade（2026-07-19｜從 root SPEC.md v1.0 升級）

> 本章由 v3.0 stub upgrade 流程新增。在 v2.2.1 sweet-spot-driven rewrite 基礎上補強 2026-07-19 重新評估、North Star Metric、Stage Gate 與 Open Questions。所有先前章節（§1–§15）皆有效，v3.0 不替換既有結論，僅結構補強 + 千字補充。

### 16.1 版本歷程與本次升級原因

- **v1.0（root SPEC.md stub，遠端實測 473 bytes / 任務指稱 233 bytes）**：schema = 產品概覽 / 目標用戶 / 核心功能 / 技術架構；無 KPI、Non-Goals、Stage Gate、AC、SOP、ADR。
- **v2.2.1（PRD/SPEC.md）**：sweet-spot-driven rewrite，14 個要求區塊、10 AC、ADR、SOP 與證據。
- **v3.0（本版）**：在 v2.2.1 之上補強結構（North Star Metric、Stage Gate、Open Questions），並依 2026-07-19 競品 quick check 重新評估 sweet spot。

### 16.2 North Star Metric（v3.0 新增）

> **「每週完成 N 次面試後證據化複盤（recorded post-interview debrief）的有效使用者人數」**

- **定義**：使用者在自身同意下錄製/匯入真實面試音檔或轉錄，由 AI 給出可分證據/推論的回饋；每週完成 ≥1 次為 1 個 WAU。
- **為何不是它**：不以「模擬面試總次數」為 North Star，因為模擬次數無法代表真實面試改善；不以「付費訂閱數」為 North Star，因為過早營收優化易犧牲普惠性。
- **目標節奏**：Discovery 2 週內 3 位 WAU；MVP 4 週內 15 位 WAU；M6 達 100 位 WAU。
- **失敗訊號**：連續 4 週 WAU 成長 < 10%，檢查 §15.10 行動建議是否觸發 Pivot/Hold。

### 16.3 Stage Gates（v3.0 新增）

| Stage | 進入條件 | 退出條件（go） | 失敗條件（pivot / hold / exit） |
|---|---|---|---|
| Stage 0（landing smoke） | PRD v3.0 落地 | ≥5 位陌生訪客完成 hero CTA | < 1 位訪客完成 CTA → stage 1.5 強制 5 訪談 |
| Stage 1（5 訪談） | Stage 0 退出 | ≥3 位證實痛點且同意 pilot | < 2 位證實痛點 → pivot wedge 或 hold |
| Stage 1.5（社群 smoke，sweet<5 強制） | Stage 1 完成 | 公開文案 / Reddit / Threads / Dcard / PTT Tech_Job 收集 ≥10 個對話訊號 | 訊號 < 3 個 → 重新定位 niche |
| Stage 2（5 位 pilot） | Stage 1.5 退出 | ≥3 位 pilot 完成 2 次練習並願意匯入真實資料 | < 2 位 pilot 完成 2 次 → hold |
| Stage 3（MVP 公開） | Stage 2 退出 | WAU ≥ 15、付費/合作 ≥3 個訊號 | WAU 連續 4 週 < 10% → pivot |
| Stage 4（成長） | Stage 3 退出 | 週留存 ≥ 30%、LTV/CAC ≥ 3 | LTV/CAC < 1 → 重新評估 niche 或 exit |

### 16.4 2026-07-19 競品 quick check 與 niche 重新驗證

| 競品 / 來源 | HTTP | 評估 |
|---|---|---|
| GitHub topics: mock-interview | 200 | 大量 dev-tools 開源實作，但無 dominant player |
| GitHub topics: ai-interview | 200 | 多為代答/作弊導向，與本產品 niche 錯開 |
| Pramp（pramp.com） | 200 | 真人 mock interview；繁中、科技業情境未被針對性服務 |
| Interviewing.io（interviewing.io） | 200 | 矽谷 FAANG 導向；價位高、繁中零覆蓋 |
| Final Round AI（finalroundai.com） | 200 | 通用代答輔助；已知規模約 1,000 萬 users；本產品避開代答 niche |
| Google 搜尋：site:jobs.104.com.tw AI mock interview | 200 | 證實繁中模擬面試討論存在於 PTT / Dcard / Threads |
| Reddit r/cscareerquestions mock interview | 200 | 英文系為主；繁中科技業面試未被整合 |

### 16.5 第三次 sweet spot re-check（v3.0）

- **Q1 痛點（求職者面試前後焦慮 / 可信複盤）**：6/10。Final Round AI ~1,000 萬 users 證實痛點存在；但台灣真實面試錄音複盤驗證仍待 §11 完成。
- **Q2 競品空隙（繁中 + 職缺客製 + 證據化複盤）**：6/10。Pramp / Interviewing.io / Yoodli / Final Round AI 都未針對繁中科技業混語情境。
- **Q3 付費意願（台灣轉職者）**：5/10。教練市場 3,000-8,000 NTD/月存在，但繁中 AI 月費 199-499 NTD 願付訊號待驗。
- **Q4 一人維護（Chrome extension + LLM API + web static）**：7/10。架構簡潔；唯一痛點是繁中語料微調要時間。
- **Q5 法規與倫理（無代答無作弊）**：6/10。預設低風險；consent 與一鍵刪除機制需技術驗證。
- **sweet = (6 + 6 + 5 + 7 + 6) / 5 = 6.0**
- **商業化 = 30 + 6 × 7 = 72 / 100**
- **行動 = INVESTIGATE**：sweet 在可投資區間，不靜默 die；但仍須依 §16.3 Stage Gate 走完 Discovery 才能正式轉 go。

### 16.6 Open Questions（v3.0 新增）

1. 繁中科技業面試中，中英混用語境比例為何？需訪談量化（§11 target）。
2. Chrome extension side-panel 的「明示式提示（不自動代答）」是否在 v3.0 列入 P1？— 仍依本文件 §3.2 採 3+ 相同請求才排入。
3. 校園職涯中心若採用 cohort dashboard（§3.2 P1-03），是否影響隱私設計？需獨立 DPA 與去識別化規則。
4. 錄音技術若失敗，使用者是否能貼上文字完成同一份複盤（AC-005）？— 必為 P0，不可降級。
5. 即時教練（§3.1 FR-008）預設關閉是否合理？— 仍預設關閉，並在 UI 顯示使用規範與「請用自己的話」。
6. 第三次 sweet spot re-check 是否要在 Stage 2 結束時再次觸發？— 是，sweet < 5 不變時，重新檢查 niche。
7. v3.0 是否需 Notion-side 同步？— 否，僅維持 git-driven PRD；Notion 由 main agent 處理。

### 16.7 v3.0 SOP 與決策紀錄

- 本次 stub upgrade commit 字串：`wip(prd): v3.0 stub upgrade — ai-interview-assistant (from root SPEC)`
- 本次 stub upgrade 時間：2026-07-19
- 本次 stub upgrade 對象：`PRD/SPEC.md`（**不修改** root `SPEC.md` stub；root 仍保留原始 473 bytes v1.0）
- 本次 stub upgrade 與 v2.2.1 差異：§16 全章為新增；§1–§15 標題與 §1 表頭升級為 v3.0；§15.13 數字維持 5/10 不變，作為可審計的歷史值。
- 本次決策：INVESTIGATE（升級），啟動 §16.3 Stage 0 / Stage 1。

*文件結束。本文件為 v3.0（自 v2.2.1 升級，stub root SPEC.md v1.0 → 完整 PRD/SPEC.md v3.0）。*
