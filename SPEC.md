...
| 術語 | 定義 |
|---|---|
| 即時側邊欄 | 面試進行中在瀏覽器右側顯示 AI 對問題的即時建議 + STAR 回答框架 |
| STAR 法則 | Situation（情境）/ Task（任務）/ Action（行動）/ Result（結果）面試回答框架 |
| 模擬面試官 | AI 扮演面試官主動提問，使用者練習回答，v2 功能 |
| 履歷解析 | AI 自動從 PDF/Docx 履歷抽取技能、學歷、經歷，預填回答素材 |
| 信心分數 | AI 對辨識準確度的自我評分（0-1），低於 0.6 顯示「請用自己的話」 |
| 語音轉文字 | Web Speech API + Whisper API 雙重 fallback，處理中文口音 |
| GPT-4o | OpenAI 多模態模型，支援即時語音輸入與低延遲回應 |
| GPT-4o-mini | GPT-4o 輕量版，成本約 1/15，用於非即時任務（履歷解析、回答潤稿）|
| WebRTC | 瀏覽器原生即時通訊協定，視訊面試透過 Zoom/Google Meet 進行，本工具僅擷取音訊 |
| 企業 ATS | Applicant Tracking System 企業徵才系統，整合 API 讓 HR 直接推送面試題目 |
| 個資法 | 個人資料保護法（台灣 2010 施行，2023 修正），履歷屬敏感個資 |
| Pro 訂閱 | NT$ 199/月，含即時側邊欄 + 履歷解析 + STAR 建議 |
| 企業方案 | NT$ 5,000/月，含多帳號 + 客製題庫 + 面試報表匯出 + SSO 整合 |
| CAC | Customer Acquisition Cost 客戶獲取成本 |
| LTV | Life Time Value 客戶終生價值 |
| ARR | Annual Recurring Revenue 年度經常性收入 |
| MoM | Month over Month 月對月成長率 |
| D7 留存 | Day 7 Retention 第 7 天仍回訪的用戶比例 |

### 10.3 參考資料
- OpenAI GPT-4o 即時 API 文件
- Web Speech API 瀏覽器相容性表
- Whisper API 中文辨識準確率研究
- STAR 法則 — Google 人資長 Laszlo Bock《Work Rules!》
- Final Round AI 公開案例研究
- Interview Cake 公開題庫
- 台灣個資法（2023 修正版）
- Stripe Webhook 設計模式
...
### 12.5 模擬面試官 v2 延遲過高
**症狀**：v2 上線後用戶投訴 AI 回應太慢
**修復**：預先生成常見問題的回答模板 + WebSocket 串流回應 + 語音輸入並行處理

### 12.6 履歷解析個資疑慮
**症狀**：使用者擔心履歷被用於訓練其他模型
**修復**：明確告知資料用途（僅用於個人化建議，不訓練模型）+ 提供「不要儲存履歷」選項

### 12.7 企業 SSO 整合失敗
**症狀**：企業客戶的 Okta/Azure AD 整合失敗
**修復**：標準 SAML 2.0 + 預先測試常見 IdP + 提供 OAuth fallback

### 12.8 面試題庫抄襲疑慮
**症狀**：有競爭對手抄襲題庫
**修復**：每題加上使用者 ID 浮水印 + 對外 API 加 timestamp 簽名

### 12.9 OpenAI API 政策改變
**症狀**：OpenAI 2026-08 宣布禁止即時語音商業用途
**修復**：本地 LLM（Llama 3.1 8B）fallback + 與 OpenAI 簽企業合約保留 API 權限

### 12.10 求職淡季 D7 留存暴跌
**症狀**：農曆年後求職潮結束，D7 留存 < 15%
**修復**：加入「面試經驗複盤」功能讓已找到工作者持續使用 + 提供校友社群
...
| 風險 | 機率 | 影響 | 對沖措施 |
|---|---|---|---|
| OpenAI 禁止即時語音商業用途 | 中 | 極高 | 本地 LLM fallback + 企業合約 |
| Google/微軟推出免費 AI 面試工具 | 高 | 高 | 專注中文市場 + 累積繁中題庫護城河 |
| 求職淡季 Q1-Q2 用戶下滑 50% | 高 | 中 | 加入「面試後複盤」+ 校友社群留住用戶 |
| 個資法裁罰（履歷外洩）| 低 | 極高 | 全資料加密 + 定期第三方稽核 + ISO 27001 |
| AI 建議偏頗引發求職者投訴 | 中 | 中 | 免責聲明 + 不取代真人 HR + 提供多元建議 |
| GPT-4o 成本失控 | 中 | 中 | GPT-4o-mini fallback + Pro 用戶限次數 + 動態調整模型 |

### 15.6 退出策略（Exit Strategy）

若 M12 MRR < NT$ 50K 且成長率 < 5% MoM，將評估：
- **選項 A**：轉型為企業 HR 徵才工具（B2B only，鎖定 30 家中型企業）
- **選項 B**：打包出售給 104 / 518 / yes123 等人力銀行
- **選項 C**：縮減為開源 Chrome Extension，免費提供基礎 STAR 建議
- **決策原則**：M6 開始每月 review，M12 仍未達標則啟動評估

### 15.7 Open Questions（市調後）

| # | 問題 | 負責人 | 預計回答 |
|---|---|---|---|
| 1 | GPT-4o 即時語音 API 商業授權時程？ | 法務 + OpenAI 業務 | M1 (2026-08) |
| 2 | 求職者實際願付價格分佈？ | 100 位訪談 | M2 (2026-09) |
| 3 | 企業 HR 月付 NT$ 5,000 上限？ | 30 位 HR 訪談 | M2 (2026-09) |
| 4 | 模擬面試官 v2 推出時程？ | Sean + 工程 | M4 (2026-11) |
| 5 | 履歷加密儲存符合個資法？ | 法務顧問 | M1 (2026-08) |
| 6 | 是否拓展海外華人市場（香港/新加坡）？ | Sean | M6+ |

### 15.8 實作路線圖

**Phase 1（M1-M3）**：核心 MVP + 付費驗證
- 完成即時側邊欄 + 履歷解析 + STAR 建議
- 上線 NT$ 199/月 Pro 版
- 招募 100 位 beta 求職者
- 目標：300 註冊 / 30 付費 / NT$ 6K MRR

**Phase 2（M4-M6）**：企業版 + 模擬面試官 v2
- 上線企業版（NT$ 5,000/月）含多帳號 + 客製題庫
- 推出模擬面試官 v2
- 招募 5 家企業客戶試用
- 目標：1,500 註冊 / 150 付費 / NT$ 40K MRR

**Phase 3（M7-M12）**：規模化 + 國際化
- 累積 5,000 繁中題庫
- 支援英文版（新加坡/香港華人市場）
- 整合企業 ATS（Greenhouse / Workday）
- 目標：5,000 註冊 / 600 付費 / NT$ 150K MRR

**Phase 4（M13+）**：獲利 + 護城河
- 達到損益兩平（NT$ 200K MRR）
- 累積 50,000+ 面試模擬資料
- 與大學職涯中心合作（B2B2C）
- 目標：NT$ 500K MRR / NT$ 6M ARR

### 15.9 投資回報率（ROI）

假設總投資 NT$ 1.2M（18 個月開發 + 行銷）：
- **保守 M12**：NT$ 1.8M ARR → 0.67 年回本
- **中等 M18**：NT$ 6M ARR → 0.20 年回本
- **樂觀 M24**：NT$ 12M ARR → 0.10 年回本
- **NPV（5 年，10% discount）**：NT$ 8.5M（中等情境）
- **IRR**：312%（中等情境）

### 15.10 參考資料
- Grand View Research "AI Recruitment Market" 2025
- iResearch 台灣人力銀行市場報告 2025
- OpenAI GPT-4o Pricing & Rate Limits
- Stripe Webhook Idempotency 設計
- 個資法（2023 修正版）
- Web Speech API 中文相容性研究
- 104 / 518 / yes123 公開徵才數據

### 15.11 關鍵指標儀表板（KPI Dashboard）

每月追蹤以下指標，用以判斷是否觸發退出策略：

**獲客指標**
- 訪客數（Monthly Visitors）
- 註冊轉化率（Visitor → Signup）
- Beta → 付費轉化率（Trial → Paid）
- CAC（Customer Acquisition Cost）by channel

**活躍指標**
- DAU / MAU / WAU（每日/每月/每週活躍用戶）
- D1/D7/D30 留存率
- 平均面試模擬時長（分鐘）
- Pro 用戶月均模擬次數
...
**告警閾值**（任一觸發則啟動 pivot 評估）
- D7 留存 < 15% 連續 2 個月
- Churn > 10% 月
- MRR 成長 < 5% MoM 連續 3 個月
- CAC 上升 > 50% 連續 2 個月
- NPS < 30
...
**短期護城河（M0-M6）**
- 繁中題庫累積 1,000+ 題（競品最多 200）
- GPT-4o 中文即時辨識準確率 92%（領先 Final Round 78%）
- STAR 框架資料庫 500+ 範例

**中期護城河（M6-M12）**
- 累積 10,000+ 模擬面試資料（訓練 v2 AI 模型）
- 企業客戶黏性（NT$ 60K LTV + 整合成本 30 萬 = 切換成本高）
- 校友社群效應（NT$ 199/月 × 5,000 人 = 護城河網絡效應）

**長期護城河（M12+）**
- 繁中面試 AI 模型自訓練（與 GPT-4o 解耦）
- 與 10+ 大學職涯中心合作（B2B2C）
- 國際華人市場擴張（HK/SG/MY TW 華人 800 萬人）

### 15.13 變現里程碑時間線

| 月份 | 累計付費 | MRR | 重大里程碑 |
|---|---|---|---|
| M3 | 30 | NT$ 6K | MVP 上線 + 首批 30 位付費用戶 |
| M6 | 150 | NT$ 40K | 企業版上線 + 5 家企業客戶 |
| M9 | 350 | NT$ 90K | 模擬面試官 v2 推出 |
| M12 | 600 | NT$ 150K | 達到損益兩平（M12 月成本 NT$ 130K） |
| M15 | 1,000 | NT$ 250K | 國際化（HK/SG）啟動 |
| M18 | 1,500 | NT$ 380K | 繁中自訓練模型上線 |
| M24 | 3,000 | NT$ 750K | NT$ 9M ARR |

### 15.14 假設檢驗清單（Hypothesis Testing）

| # | 假設 | 檢驗方法 | 成功標準 | 時程 |
|---|---|---|---|---|
| H1 | 求職者願付 NT$ 199/月 | 100 位訪談 + 100 位 A/B test | ≥ 20% 願付 | M2 |
| H2 | 即時側邊欄是殺手級功能 | 50 位用戶 5 秒內測試 | ≥ 80% 認為必要 | M3 |
| H3 | STAR 建議提高面試表現 | 對照組 vs 實驗組 | 實驗組分 ≥ 對照組 +20 | M4 |
| H4 | AI 辨識延遲 < 5 秒 | 實測 50 次 | ≥ 40 次達標 | M2 |
| H5 | 企業 HR 願付 NT$ 5,000/月 | 30 位 HR 訪談 | ≥ 5 位簽約 | M6 |
| H6 | Pro 升級轉化率 ≥ 10% | funnel 分析 | 達標 | M3 |
| H7 | 校友社群留存率 ≥ 30% | 6 個月觀察 | 達標 | M12 |
| H8 | 國際華人市場接受度 | HK/SG 500 位問卷 | ≥ 15% 願付 | M18 |

任一假設連續 2 個月未達標 → 啟動產品 pivot 評估。

### 15.15 最終商業化評分

整合 §15.4 + §15.5 風險 + §15.8 路線圖後的最終評分：

| 維度 | 分數（0-100）| 加權 |
|---|---|---|
| 市場規模（100 萬台灣求職者 + 800 萬國際華人）| 75 | 25% |
| 差異化（唯一繁中即時 + STAR 整合）| 85 | 20% |
| 變現路徑（B2C + B2B + 校友社群）| 75 | 15% |
| LTV/CAC（11.9 / 20.0 兩端健康）| 90 | 15% |
| 技術可行性（GPT-4o + 本地 fallback）| 70 | 10% |
| 風險管理（5 項對沖措施）| 65 | 10% |
| 護城河（題庫 + 資料 + 校友網絡）| 70 | 5% |
| **加權平均** | **76** | 100% |

**最終商業化評分：76/100**（中高商業化潛力，主要風險為 OpenAI 政策變動 + 國際華人市場接受度待驗證）。

### 15.16 補充：產品 v1.0 已知問題與 v1.5 改進

v1.0 MVP 已於 2026-07 上線，目前累積 32 位 alpha 用戶回饋，主要問題：
1. **語音辨識延遲**：3-5 秒（目標 < 3 秒）→ v1.5 改用 WebSocket 串流
2. **STAR 建議品質**：部分用戶反映「建議太通用」→ v1.5 增加職位類型細分
3. **履歷解析**：PDF 中文亂碼 → v1.5 改用 pdf-parse + 雙重 OCR fallback
4. **行動版體驗**：手機面試模擬 UX 不佳 → v1.5 PWA + 簡化版 UI
5. **付費牆**：用戶反映「沒試用就收費」→ v1.5 改為 7 天免費試用 + 不自動續訂

### 15.17 補充：行銷通路 ROI 預估

| 通路 | 預估 CAC | 預估轉化率 | 月獲客上限 | 備註 |
|---|---|---|---|---|
| 求職 FB 社團 | NT$ 80 | 3% | 500 | 免費貼文 + 偶爾付費推廣 |
| Instagram Reels | NT$ 150 | 2% | 300 | 需持續產出短影音內容 |
| LinkedIn 廣告 | NT$ 500 | 4% | 100 | B2B 企業客戶為主 |
| Google Ads "面試準備" | NT$ 250 | 5% | 200 | 意圖明確，CAC 中等 |
| 大學校園徵才 | NT$ 100 | 8% | 800 | B2B2C 模式，LTV 高 |
| 部落客開箱 | NT$ 50 | 6% | 150 | 找 5-10 位求職 KOL 合作 |
| 校友社群推薦 | NT$ 30 | 15% | 200 | 口碑效應，ROI 最高 |
